...
- 컴퓨터(Computer)
    - 1부터 9까지의 서로 다른 임의의 수를 3개 선택한다. (Randoms API)

- 사용자(Player)
    - 1부터 9까지의 서로 다른 임의의 수를 3개 입력한다. (Console API)
    - 잘못된 값을 입력할 경우 IllegalArgumentException을 발생시킨 후 애플리케이션을 종료한다.
    - 게임 종료 후 재시작(1) 또는 종료(2)를 입력한다.

- 심판(Umpire)
    - 컴퓨터가 선택한 수와 플레이어가 선택한 수를 비교하여 게임 결과를 판별한다.