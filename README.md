...
## 기능 목록
1. 하나의 숫자가 입력되면 그 숫자가 스트라이크인지, 볼인지, 낫싱인지 판단한다.
2. 여러 숫자가 입력되면 각 숫자마다 스트라이크인지, 볼인지, 낫싱인지 판단한다.
3. 여러 숫자가 입력된 최종 결과가 성공인지 실패인지 판단한다.

~~4. 랜덤으로 하나의 숫자를 생성한다.~~

~~5. 랜덤으로 여러 개의 숫자를 생성한다.~~

6. 한 숫자가 입력되면 게임에서 요구하는 조건을 충족하는지 판단한다.
7. 여러 숫자가 입력되면 게임에서 요구하는 조건을 충족하는지 판단한다.
8. 게임이 끝나면 다시 시작할지 완전 종료할지 여부를 사용자가 결정할 수 있도록 UI를 제공한다.
9. 사용자의 입력값이 게임에서 요구하는 조건을 충족하지 않을 경우 에러 메시지를 출력하고 게임을 다시 시작한다.
