...
public class Scanner {
    ...
    private static void validateCommandInput(final String input) {
        validateCommandValue(input);
    }

    private static void validateSwingInput(final String input) {
        isNumber(input);
        isSameNumber(input);
        validateSwingLength(input);
    }

    private static void validateCommandValue(final String input) {
    ...
    private static void isSameNumber(final String input) {
        if (Pattern.compile(SAME_NUMBER_PATTERN).matcher(input).find()) {
            throw new IllegalArgumentException("서로 다른 숫자를 입력해야합니다.");
        }
    }

    private static void validateSwingLength(final String input) {
