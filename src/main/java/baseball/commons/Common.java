...
public class Common {
     ...
     * @param max    문자열 속 가장 큰 숫자의 문자 제한
     * @param length 문자열 길이 제한
     * @return 입력된 문자열의 정상 여부
     */
    public static Boolean isValidString(String input, Integer max, Integer length) {
        try {
            isValidInputString(input, max, length);
            isValidDuplicateChar(input, length);
     ...
     */
    private static void isValidInputString(String input, Integer max, Integer length) {
        if (!input.matches(String.format(Constant.ONLY_NUMBER_VALIDATE_REGEX, max, length))) {
            throw new IncorrectInputException(String.format("입력값이 옳지 않습니다.(%d자리의 1~%d 사이의 숫자만 가능)", length, max));
        }
    }
     ...
     */
    private static void isValidDuplicateChar(String input, Integer length) {
        String newInput = "";
    ...
    public static Boolean isContinueGame() {
        ...
        } while (!isValidString(input, Constant.GAME_RESTART_FLAG_MAX, Constant.GAME_RESTART_FLAG_LENGTH));
